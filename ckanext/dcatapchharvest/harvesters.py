...
class SwissDCATRDFHarvester(DCATRDFHarvester):
    ...
    def validate_config(self, source_config):
        ...
        if "excluded_dataset_identifiers" in source_config_obj:
            ...
            if not all(isinstance(item, str) for item in excluded_dataset_identifiers):
                raise ValueError(
                    "excluded_dataset_identifiers must be " "a list of strings"
                )
        ...
        if "excluded_license" in source_config_obj:
            ...
            if not isinstance(excluded_license, list):
                raise ValueError("excluded_license must be " "a list of strings")
            if not all(isinstance(item, str) for item in excluded_license):
                raise ValueError("excluded_license must be " "a list of strings")
...
class SwissDCATI14YRDFHarvester(SwissDCATRDFHarvester):
    ...
    def _get_guid(self, dataset_dict, source_url=None):
        guid = super(SwissDCATI14YRDFHarvester, self)._get_guid(
            dataset_dict, source_url
        )
